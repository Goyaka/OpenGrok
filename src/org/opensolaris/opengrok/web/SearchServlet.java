...
public class SearchServlet extends HttpServlet {
	...
	private void renderJSONResponse() throws IOException{
		...
		LinkedHashMap<Integer, SearchResult> results = new LinkedHashMap<Integer, SearchResult>();
		
		for (ScoreDoc hit : hits) {
			SearchResult result = new SearchResult();
			int documentId = hit.doc;
			result.path = searcher.doc(documentId).get("path");
			result.hit = hit;
		...
		try { 
			...
			if (env.hasProjects()) {
				...
				if (q != null || defs != null || refs != null || hist != null || path != null) {
					...
					// Build the query
					QueryBuilder queryBuilder = new QueryBuilder().setFreetext(q).setDefs(defs).setRefs(refs).setPath(path).setHist(hist);
					query = queryBuilder.build();

					// Find and set the appropriate Searcher class for this query
					setSearcher();
